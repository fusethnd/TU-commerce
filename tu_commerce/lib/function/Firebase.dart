...
Future<void> saveHistory(order, id, user, type) async {
  ...
  if (id == 'No') {
    ...
    querySnapshot.docs.forEach((doc) {
      doc.reference.update({'historyID': docID.toString()});
    });
  } else {
    docID = id.toString();
  }
...
Future<void> updateStatus(order, status, index) async {
  // update field status
  QuerySnapshot querySnapshot = await FirebaseFirestore.instance
      .collection('Orders')
      .where('product.prodID', isEqualTo: order['product']['prodID'])
      .where('username.username', isEqualTo: order['username']['username'])
      .get();
  order['status'] = status;
...
Future<List<QueryDocumentSnapshot<Object?>>> getOrders(
    username, shoppingMode) async {
  // หยิบตาม shopping mode ถ้าเกิดเป็นโหมดคนซื้อก็จะหาแค่อันที่ตัวเองซื้อ ถ้าโหมดคนขายก็จะหาแค่โหมดที่ตัวเองได้ request
  QuerySnapshot querySnapshot;
  if (shoppingMode) {
    querySnapshot = await FirebaseFirestore.instance
        .collection('Orders')
        .where('username.username', isEqualTo: username)
        .get();
  } else {
    querySnapshot = await FirebaseFirestore.instance
        .collection('Orders')
        .where('product.seller.username', isEqualTo: username)
        .get();
  }
...
Future<List<dynamic>?> updateUser(Map<String, dynamic> user,
    Map<String, dynamic>? product, String status) async {
...
bool isFavorite(Map<String, dynamic>? name, List<dynamic>? fav) {
  // check ว่ามี item นี้อยุ่แล้วมั้ย
  if (fav == null) {
    return false;
  } else {
    for (var item in fav!) {
      if (item is Map<String, dynamic> && item['prodID'] == name!['prodID']) {
        // productID ใช้ check
        return true;
      }
    }
...
Future<Map<String, dynamic>> getHistory(String id) async {
  DocumentSnapshot<Map<String, dynamic>> querySnapshot =
      await FirebaseFirestore.instance.collection('History').doc(id).get();
  Map<String, dynamic> historyData = querySnapshot.data()!;
  return historyData;
}

Future<void> sendNotificationToUser(
    tokenCustomer, String notificationTitle, String notificationBody) async {
  ...
  if (recipientDeviceToken != null) {
    const String serverKey =
        'AAAAJyGO45I:APA91bEAm0BD51p3onfptJ_oz7s90U4f-nLF0JnKdVOC1FwUPyYiX64-jRuDWc8iF6F3WTQH92-BZoteiBcXPyoDfxNXSQ1qws5-jZd0DkK0IDpejkzA6G4mDAAhJEVteGUxqaD_3jKY'; // Replace with your FCM server key
